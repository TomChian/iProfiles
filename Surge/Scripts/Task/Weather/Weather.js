...
const address = "&location=N,E";
const k = "&key=xxxxxxxxxxxxxxxxxxxcccccxxxxxxxx";

const wea = "https://free-api.heweather.net/s6/weather/now?"+address+k;
const forecast = "https://widget-api.heweather.net/s6/plugin/sticker?key=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx&location="+$persistentStore.read("cid")+"&lang=zh";
...
var title = "士不可以不弘毅，任重而道远！"
var subtitle = $persistentStore.read("city")+"•丰台 : "+$persistentStore.read("noweather")+" · "+$persistentStore.read("tmp")+" °C "+" · "+$persistentStore.read("ssd")+"  AQI : "+$persistentStore.read("aqi")+"("+$persistentStore.read("qlty")+")";
var mation = "实时天气预警 : "+$persistentStore.read("minute_forecast")+"\n风向风力 : "+$persistentStore.read("wind_dir")+" · "+$persistentStore.read("wind_sc")+" 级"+"  湿度 : "+$persistentStore.read("hum")+"  PM2.5 : "+$persistentStore.read("pm25")+"\n生活建议 : "+$persistentStore.read("life");
